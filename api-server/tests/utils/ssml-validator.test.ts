...
describe('SSMLValidator', () => {
  ...
  describe('validate', () => {
    it('应该验证有效的 SSML', () => {
      const validSSML =
        '<speak version="1.0" xml:lang="zh-CN" xmlns="http://www.w3.org/2001/10/synthesis"><voice name="zh-CN-XiaoxiaoNeural">你好世界</voice></speak>';
    ...
    it('应该检测 MSTTS 扩展缺少命名空间', () => {
      const invalidSSML =
        '<speak><voice name="zh-CN-XiaoxiaoNeural"><mstts:express-as style="cheerful">测试</mstts:express-as></voice></speak>';
    ...
    it('应该检测 MSTTS 扩展缺少 style 属性', () => {
      const invalidSSML =
        '<speak xmlns:mstts="http://www.w3.org/2001/mstts"><voice name="zh-CN-XiaoxiaoNeural"><mstts:express-as>测试</mstts:express-as></voice></speak>';
    ...
    it('应该自动修复 pitch="0st" 问题', () => {
      const invalidSSML =
        '<speak><voice name="zh-CN-XiaoxiaoNeural"><prosody pitch="0st">测试</prosody></voice></speak>';

      const result = validator.validate(invalidSSML);

      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain('检测到无效的 pitch="0st"，应该使用 pitch="+0st"');
      expect(result.fixedSSML).toBe(
        '<speak><voice name="zh-CN-XiaoxiaoNeural"><prosody pitch="+0st">测试</prosody></voice></speak>'
      );
    });
    ...
    it('应该检测缺少 voice 元素', () => {
      ...
      expect(result.warnings).toContain(
        'SSML 内容中缺少 <voice> 元素，建议添加 voice 元素包装内容'
      );
    });
  ...
  describe('边界情况', () => {
    ...
    it('应该处理自闭合标签', () => {
      const selfClosingSSML =
        '<speak><voice name="zh-CN-XiaoxiaoNeural"><break time="500ms"/>测试</voice></speak>';
    ...
    it('应该处理多个 pitch="0st" 问题', () => {
      const multiplePitchSSML =
        '<speak><voice name="zh-CN-XiaoxiaoNeural"><prosody pitch="0st">第一段</prosody><prosody pitch="0st">第二段</prosody></voice></speak>';
