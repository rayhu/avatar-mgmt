...
function loadEnvironmentVariables() {
  ...
  const ADMIN_PASSWORD =
    env.ADMIN_PASSWORD || env.DIRECTUS_ADMIN_PASSWORD || process.env.DIRECTUS_ADMIN_PASSWORD || '';
  ...
  if (!ADMIN_PASSWORD) {
    throw new Error('DIRECTUS_ADMIN_PASSWORD 未配置，请在 .env.directus 文件中设置');
  }
...
async function getOrCreatePublicPolicy(token) {
  // 检查是否已有 Public 策略
  const res = await fetch(`${DIRECTUS_URL}/policies?filter[name][_eq]=$t:public_label`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const data = await res.json();
