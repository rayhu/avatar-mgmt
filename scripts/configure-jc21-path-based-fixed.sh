...
create_path_based_config() {
    ...
    # 检查是否已存在配置
    local existing_id=$(sqlite3 "$DB_PATH" "SELECT id FROM proxy_host WHERE domain_names LIKE '%$domain%' LIMIT 1;")
    if [ ! -z "$existing_id" ]; then
        log_warning "发现已存在的配置 (ID: $existing_id)，将删除旧配置"
        sqlite3 "$DB_PATH" "DELETE FROM proxy_host WHERE id = $existing_id;"
    fi
...
show_config_info() {
    ...
    echo
    echo "📊 管理命令:"
    echo "   查看配置: sqlite3 ./jc21/data/database.sqlite 'SELECT * FROM proxy_host;'"
